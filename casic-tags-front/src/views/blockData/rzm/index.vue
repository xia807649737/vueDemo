  ...
  <div class="mask">
    <div class="app-container">
      <div class="label-search">
        ...
        <el-input v-model="categoryName" class="search-width230 margin-right15" size="small" clearable />
      ...
      <div class="table">
        <el-table height="calc(100vh - 350px)" :data="tableData" style="width: 100%;">
          ...
          <el-table-column prop="categoryName" label="块ID" align="center" />
          <el-table-column prop="categoryName" label="块名称" align="center" />
          <el-table-column prop="peopleNum" label="子块数量" align="center">
            <template slot-scope="scope">
              {{ scope.row.peopleNum || 0 }}
            </template>
          </el-table-column>
          <el-table-column prop="peopleNum" label="标签数量" align="center">
            <template slot-scope="scope">
              {{ scope.row.peopleNum || 0 }}
            </template>
          </el-table-column>
...
<script>
import { getlb, postlb, putlb, dellb } from '@/api/genre'
...
export default {
  ...
  methods: {
    ...
    handleSubmit: async function(formData) {
      if (!this.isEdit) {
        const res = await postlb(formData)
        if (res?.code === 200) {
        ...
        const res = await putlb(updateData)
        if (res?.code === 200) {

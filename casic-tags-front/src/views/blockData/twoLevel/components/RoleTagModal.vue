  ...
  >
    <div class="form-body">
      <el-form ref="ruleForm" label-position="right" :rules="rules" label-width="80px" :model="formRole">
        <el-form-item label="块数据根" prop="typeName">
          <el-input v-model="formRole.typeName" placeholder="类别名称" />
        </el-form-item>
        <el-form-item label="块ID" prop="categoryId">
          <el-select v-model="formRole.categoryId" style="width: 100%">
            <el-option v-for="item in options" :key="item.categoryId" :value="item.categoryId" :label="item.categoryName" placeholder="请选择子类类别" />
          </el-select>
        </el-form-item>
        <el-form-item label="块名称" prop="typeName">
          <el-input v-model="formRole.typeName" placeholder="类别名称" />
        </el-form-item>
        <div class="form-tags">
...
<script>
import { getlball } from '@/api/genre'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        typeName: [{ required: true, message: '请输入子类名称', trigger: 'blur' }],
        categoryId: [{ required: true, message: '请选择子类类别', trigger: 'change' }],
        particleDTOList: [{ required: true, message: '请选择标签', trigger: 'blur' }],
        countRule: [{ required: true, message: '请选择计算规则', trigger: 'change' }]
      }
    }
  },
  watch: {

    ...
    <div class="app-container">
      <div class="label-search">
        <div class="search-btn">
          <el-button type="primary" size="small" @click="addRole">新增分级</el-button>
        </div>
        <span class="search-label">分级名称：</span>
        <el-input v-model="attentionLevelName" class="search-width230 margin-right15" size="small" />
      ...
      <div class="table">
        <el-table height="calc(100vh - 350px)" :data="tableData" style="width: 100%;">
          ...
          <el-table-column prop="attentionLevelName" label="分级名称" align="center" />
...
<script>
import { getfj, postfj, putfj, delfj } from '@/api/genre'
  ...
  methods: {
    ...
    getTableList() {
      const data = {
        ...
        attentionLevelName: this.attentionLevelName
      }
      getfj(data).then(res => {
        if (res.code === 200) {
    ...
    onDelete(row) {
      this.$confirm('确认删除吗?', '提示', {
        ...
        delfj(row.attentionLevelId).then(res => {
          if (res.code === 200) {
    ...
    handleSubmit: async function(formData) {
      ...
      if (!this.isEdit) {
        const res = await postfj({ ...formData, associateParticleIds: ids.join(',') })
        if (res?.code === 200) {
        ...
        const updateData = {
          ...formData,
          attentionLevelId: this.current.attentionLevelId,
          associateParticleIds: ids.join(',')
        }
        const res = await putfj(updateData)
        if (res?.code === 200) {

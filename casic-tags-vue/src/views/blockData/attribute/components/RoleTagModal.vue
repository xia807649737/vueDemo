<template>
  ...
  >
    <el-form ref="ruleForm" label-position="right" :rules="rules" label-width="100px" :model="formRole">
      <el-form-item label="块数据根" prop="rootBlockName">
        <el-input v-model="formRole.rootBlockName" placeholder="块数据根" disabled />
      </el-form-item>
      <!-- <el-form-item label="块数据根ID" prop="categoryName">
        <el-input v-model="formRole.categoryName" placeholder="块数据根ID" />
      </el-form-item> -->
      <el-form-item label="属性名称" prop="rootBlockAttrName">
        <el-input v-model.trim="formRole.rootBlockAttrName" placeholder="属性名称" />
      </el-form-item>
    </el-form>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        rootBlockAttrName: [{ required: true, message: '请输入类别名称', trigger: 'blur' }]
      }
    }
  },
  watch: {

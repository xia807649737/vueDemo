  ...
  >
    <div class="form-body">
      <el-form ref="ruleForm" label-position="right" :rules="rules" label-width="100px" :model="formRole">
        <el-form-item label="根块数据" prop="parentBlockName">
        ...
        <el-form-item label="块数据属性" prop="rootBlockAttrCode">
          <el-select v-model="formRole.rootBlockAttrCode" style="width: 100%" :disabled="!!formRole.blockCode">
            <el-option v-for="item in options" :key="item.rootBlockAttrCode" :value="item.rootBlockAttrCode" :label="item.rootBlockAttrName" placeholder="请选择块数据属性" />
          </el-select>
        </el-form-item>
...
import { atrList, details } from '@/api/block'
  ...
  data() {
    return {
      ...
      rules: {
        ...
        rootBlockAttrCode: [{ required: true, message: '请选择块数据属性', trigger: 'change' }]
      }
    }
  },
  watch: {
    ...
    visible(val) {
      ...
      if (val) {
        ...
        if (this.formRole.parentBlockCode) {
          const id = this.formRole.parentBlockCode
          atrList(id).then(res => {
            if (res?.code === 200 || res?.status === '0') {

...
<script>
import { getl, postl, dell, putl, jsnum, ulstatus, findTree } from '@/api/block'
...
export default {
  ...
  methods: {
          ...
          for (let i = 0; i < list.length; i++) {
            ...
            if (children && children.length) {
              for (let j = 0; j < children.length; j++) {
                const it = children[j]
                it.id = it.blockCode
                it.label = it.blockName
                const chil = it.children
                if (chil && chil.length) {
                  for (let k = 0; k < chil.length; k++) {
                    const itt = chil[k]
                    delete itt.children
                    itt.id = itt.blockCode
                    itt.label = itt.blockName
                  }
                } else {
                  delete it.children
                  it.isDisabled = true
    ...
    getTableList(num, params) {
      ...
      const data = {
        parentBlockCode: this.obj && this.obj.blockCode,
    ...
    // 新增角色
    addRole() {
      if (!this.obj || !this.obj.id) return this.$message.warning('请选择二级块数据！')
      this.current = { parentBlockCode: this.obj.blockCode, parentBlockName: this.obj.blockName, rootBlockAttrCode: this.obj.rootBlockAttrCode }
      this.roleVisible = true
      this.isEdit = false
    },
    // 分页
    ...
    handleSubmit: async function(formData) {
      ...
      if (!this.isEdit) {
        const res = await postl({ ...formData, associateParticleIds, blockLevel: 3, rootBlockCode: this.obj.rootBlockCode })
        if (res?.code === 200 || res?.status === '0') {
        ...
        const updateData = {
          ...
          rootBlockCode: formData.rootBlockCode,
          blockLevel: 3,
    ...
    handleSelect(node) {
      this.getTableList(1, { parentBlockCode: node.blockCode })
    }
  }
}
</script>

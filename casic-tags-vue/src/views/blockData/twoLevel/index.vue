...
export default {
  ...
  methods: {
    ...
    // 新增角色
    addRole() {
      if (!this.obj || !this.obj.id) return this.$message.warning('请选择一块数据！')
    ...
    handleSubmit: async function(formData) {
      ...
      if (!this.isEdit) {
        const res = await postl({ ...formData, associateParticleIds, blockLevel: 2, rootBlockCode: this.obj.rootBlockCode })
        if (res?.code === 200 || res?.status === '0') {

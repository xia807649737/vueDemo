        ...
        <div class="table">
          <div class="label-search">
            <div class="search-btn">
              <el-button type="primary" size="small" @click="addRole">新增颗粒</el-button>
            </div>
          ...
          <el-table v-loading="loading" :data="tableData" height="calc(100vh - 340px)" style="width: 100%">
            <el-table-column prop="atomId" label="颗粒ID" align="center" />
            <el-table-column prop="atomName" label="颗粒名称" align="center" show-overflow-tooltip />
            <el-table-column prop="atomLevel" label="层级" align="center" />
            <el-table-column prop="peopleNum" label="人员数量" align="center">
              <template slot-scope="scope">
                <span>{{ scope.row.peopleNum || 0 }}</span>
              </template>
            </el-table-column>
            <el-table-column prop="atomCallNum" label="累计达标数" align="center">
              <template slot-scope="scope">
                <span>{{ scope.row.atomCallNum || 0 }}</span>
              </template>
            </el-table-column>
            <el-table-column prop="atomKeyPerson" label="重点人标签" align="center">
              <template slot-scope="scope">
                <span>{{ scope.row.atomKeyPerson == '1' ? '是' : '否' }}</span>
              </template>
            </el-table-column>
            <el-table-column prop="atomLegal" label="法定标签" align="center">
              <template slot-scope="scope">
                <span>{{ scope.row.atomLegal == '1' ? '是' : '否' }}</span>
              </template>
            </el-table-column>
            <el-table-column prop="taskStatus" label="标签状态" align="center">
              <template slot-scope="scope">
                <span>{{ scope.row.taskStatus == '1' ? '启用' : '停用' }}</span>
              </template>
            </el-table-column>
            <el-table-column prop="ext2" label="打标周期" align="center">
              <template slot-scope="scope">
                <span>{{ getName(scope.row.ext2) }}</span>
              </template>
            </el-table-column>
            <el-table-column prop="updateTime" label="最后修改时间" align="center">
              <template slot-scope="scope">
                <span>{{ (scope.row.updateTime || scope.row.createTime) && moment(scope.row.updateTime || scope.row.createTime).format('YYYY/MM/DD HH:mm:ss') }}</span>
              </template>
            </el-table-column>
...
import { getArr } from './components/tree'
